...
class PyStockBotPipeline:
    ...
    def __init__(self, data_dir: str = "../../data", symbol: str = "AAPL"):
    ...
    def run_data_ingestion(self, years_back: int = 5, force_refresh: bool = False):
        """
    ...
    def run_target_creation(self, prediction_horizons: list = None):
        """
    ...
    def run_model_training(self, target_columns: list, perform_tuning: bool = False):
        """
    ...
    def run_full_pipeline(self, years_back: int = 5, 
...
def main():
    ...
    operations = {
        'analyze_only': lambda: (
            pipeline._load_existing_results() and
            pipeline.analyze_results() is None  # analyze_results returns None, so we use 'is None' for success
        ),
        'full_pipeline': lambda: (
    ...
    mode = 'analyze_only' if args.analyze_only else 'full_pipeline'
    
    try:
        success = operations[mode]()
